    ...
    DisposableEffect(player) {
        val listener = object : Player.Listener {
            ...
            override fun onPlayerError(error: PlaybackException) {
                LogMessage.e("ExoPlayer Error: ${error.message}")
            }
        }
                    ...
                    IconButton(
                        ...
                        activity?.let { act ->
                            ...
                            trimVideo(
                                ...
                                onError = {
                                    isProcessing = false
                                    Toast.makeText(act, "Failed to trim", Toast.LENGTH_SHORT).show()
                                }
                            )
                        }
                    }) {
...
private fun getFileName(context: Context, filePath: Uri): String {
    ...
    val fileDateTime = "${calendar.get(Calendar.YEAR)}_${calendar.get(Calendar.MONTH)}_${calendar.get(Calendar.DAY_OF_MONTH)}_${calendar.get(Calendar.HOUR_OF_DAY)}_${calendar.get(Calendar.MINUTE)}_${calendar.get(Calendar.SECOND)}"
    val fName = "trimmed_video_"
    val newFile = File(path + File.separator + fName + fileDateTime + "." + TrimmerUtils.getFileExtension(context, filePath))
